...
def output_data_to_file_text(output):
    ...
    with open(output, "w") as f:
        for device in usb_devices:
            ...
            if device.device_classes_datetime_53f56307b6bf11d094f200a0c91efb8b != datetime.min:
                f.write("Device Classes Timestamp (53f56): " + str(device.device_classes_datetime_53f56307b6bf11d094f200a0c91efb8b.strftime('%Y-%m-%dT%H:%M:%S')) + '\n')
            if device.device_classes_datetime_10497b1bba5144e58318a65c837b6661 != datetime.min:
                f.write("Device Classes Timestamp (10497): " + str(device.device_classes_datetime_10497b1bba5144e58318a65c837b6661.strftime('%Y-%m-%dT%H:%M:%S')) + '\n')
            if device.vid_pid_datetime != datetime.min:
                f.write("VID/PID Timestamp: " + str(device.vid_pid_datetime.strftime('%Y-%m-%dT%H:%M:%S')) + '\n')
            if device.usb_stor_datetime != datetime.min:
            ...
            if device.usbstor_datetime65 != datetime.min:
                f.write("USBSTOR Timestamp (65): " + str(device.usbstor_datetime65.strftime('%Y-%m-%dT%H:%M:%S')) + '\n')
            if device.usbstor_datetime66 != datetime.min:
                f.write("USBSTOR Timestamp (66): " + str(device.usbstor_datetime66.strftime('%Y-%m-%dT%H:%M:%S')) + '\n')
            if device.usbstor_datetime67 != datetime.min:
                f.write("USBSTOR Timestamp (67): " + str(device.usbstor_datetime67.strftime('%Y-%m-%dT%H:%M:%S')) + '\n')
            ...
            for i in range(len(device.mountpoint2)):
                ...
                if device.mountpoint2[i].timestamp != datetime.min:
                    f.write('MP2 Timestamp: ' + str(device.mountpoint2[i].timestamp.strftime('%Y-%m-%dT%H:%M:%S')) + '\n')
...
def process_log_file(file):
    regexXp1 = b'^\[([0-9]+/[0-9]+/[0-9]+\s[0-9]+:[0-9]+:[0-9]+)\s[0-9]+.[0-9]+\sDriver\sInstall\]'
    regexXp2 = b'#-019 .*? ID\(s\): usb\\(.+)'
    regexXp3 = b'#I121.*? "(.*)"'
    ...
    while index < len(lines) - 1:
        ...
        if os_version == WindowsVersions.WindowsXP or os_version == WindowsVersions.WindowsXPx64:
            ...
            if b'USB\\' not in key and b'USBSTOR\\' not in key:
                continue

            if key in install_times is False:
                install_times[key] = timestamp

        elif os_version == WindowsVersions.WindowsVista:
            if not b'>>>  [Device Install (Hardware initiated) - USBSTOR' in line:
                continue

            match = re.search(regexVista1, line, re.I)
            if match:
                key = b'USBSTOR\\' + match.group(1)
            ...
            if not b'>>>  [Device Install (Hardware initiated) - SWD\\WPDBUSENUM\\_??_USBSTOR#' in line:
                continue

            #'>>>\s\s\[Device\sInstall\s\(Hardware\sinitiated\) - SWD\\WPDBUSENUM\\_\?\?_USBSTOR#(.*)\]'
            match = re.match(b'>>>\s\s\[Device\sInstall\s\(Hardware\sinitiated\) - SWD\\\\WPDBUSENUM\\\\_\?\?_USBSTOR#(.*)\]', line, re.I)
            if match:
                key = b'USBSTOR\\' + match.group(1)
    ...
    for key, timestamp in install_times.items():
        for device in usb_devices:
            if os_version == WindowsVersions.WindowsXP or os_version == WindowsVersions.WindowsXPx64:
                ...
                if len(parts) == 4:
                    temp = (parts[0] + "#" + parts[1] + "#" + parts[2] + '#{53f56307-b6bf-11d0-94f2-00a0c91efb8b}').encode('utf-8')

                    if temp.lower() in key.lower():
                        write_debug(data='Matched install log timestamp using mountpoint: ' + key.lower().decode())
                        device.install_datetime = datetime.strptime(timestamp.decode(), "%Y/%m/%d %H:%M:%S.%f")
                    else:
                        write_debug(data='Unable to match install log timestamp using mountpoint: ' + key.lower().decode())
                else:
...
def remove_non_ascii_characters(data):
    ...
    return ''.join([chr(i) if (i < 128 and i > 0) else '' for i in data])
